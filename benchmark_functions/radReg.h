...
namespace hicam
{
  ...
  public:
    ...
    {
      this->number_of_objectives = 2; // hardcoded
    ...
    void get_param_bounds(vec_t & lower, vec_t & upper) const
    {
      ...
      upper.clear();
      upper.resize(number_of_parameters, 20); //should make value 20 right?
    ...
    void define_problem_evaluation(solution_t & sol)
    {
      ...
      */
      ...
      for (int radial = 0; radial<K; radial++){
        int index = 3 * radial;
        stability += pow(sol.param[index],2);
      }
